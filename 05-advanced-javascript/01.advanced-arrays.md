...
This is pretty much what we were doing with our arrays, right? The main difference is that `map` already returns a new array with the result of passing a function to each one of the elements of our array. So let's see it in action:
...
2. Then, we've **declared a new function** that **receives** a number and multiplies it by 2, **returning** the result.
...
As `map` does, `filter` also returns a different array that the one we are working with, with the exception that it will contain only the elements that the expression of our callback function returns `true` for.
...
The `Reducer` function works pretty much the same as the other two. It accepts a function as a parameter exactly the same way as `map` and `filter` do, with the exception that this callback function (called reducer), instead of receiving the current element of the array as first parameter, it takes as first argument the accumulated result (accumulator) of the previously executed iterations. Let's see how [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce) defines this method and then we'll see it with some code:
...
```js
// Tip: You can chain map and filter
